...
from class_alignment import Fingerprints
from class_doctrees import DocTrees
...
def generateLink(docIntervalTree, threshold):
    ...
    for comparison in docIntervalTree.keys():
        ...
        for duplication in sorted(docIntervalTree[comparison]):
            ...
            # from,start,end
            if (duplication.end - duplication.begin) > threshold and (duplication.end, duplication.begin) not in alreadyseen :
                ...
                fromData = comparison.split("_"
                                            )[0]+","+str(duplication.begin
                                                         )+","+str(
                                                             duplication.end)
                # to,start,end
                toData = comparison.split("_"
                                          )[1]+","+str(
...
dataset = [line.replace('"','').rstrip().split('\t') for line in open("/tmp/demo.txt", "r").readlines()]
orf=3
fingerprintList=[10]
results=[]
...
for candicate in range(0,len(dataset)):
    ...
    thisDocTree.mergeOverlap("myPatientDuplication.patientTexts",
                             "keyRoot", figerprintsId.figprintId)
    print(candicate)
